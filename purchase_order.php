...
<?php
	if($office == 'delta'){
		echo "<title>Pending Orders - Purchase Order - Quality Star Concrete Products, Inc.</title>";
	}else{
		echo "<title>Pending Orders - Purchase Order - Starcrete Manufacturing Corporation</title>";
	}
?>
	<meta charset="utf-8">
...
?>
	...
	<nav class="navbar navbar-default" id="secondary-nav" style="background-color: #0884e4; margin-bottom: 10px;">
		<div class="container-fluid">
			...
			<span style="font-size:25px; color: white;">Purchase Order > Pending Orders</span>
	...
	<div id="wrapper" onclick="closeNav();">
		<div id="content">
			<form action="purchase_order.php" method="post" class="form-inline">
...
?>
						...
						<label class="radio-inline">
							<input type="radio" name="radioOffice" value="bravo" <?php if(isset($_POST['radioOffice']) && $_POST['radioOffice'] == 'bravo') { echo "checked"; }?> checked>Bravo <span class="badge"><?php echo getCountPendingOffice($db,'bravo'); ?> Order(s)</span>
						</label>
						<label class="radio-inline">
							<input type="radio" name="radioOffice" value="delta" <?php if(isset($_POST['radioOffice']) && $_POST['radioOffice'] == 'delta') { echo "checked"; }?>>Delta <span class="badge"><?php echo getCountPendingOffice($db,'delta'); ?> Order(s)</span>
						</label>
<?php
	}
?>
...
<?php
	...
	$query = "SELECT p.purchase_id, p.purchase_order_no, p.client_name, p.item_no, CONCAT(FORMAT(p.quantity,0), ' ', l.unit) as quantity, delivered, backload, CONCAT(FORMAT(balance,0), ' ', l.unit) as balance, address, contact_person, contact_no, DATE_FORMAT(date_purchase,'%m/%d/%y') as date_purchase, office, remarks
...
<?php
	...
	if(isset($_POST['cancel'])){
		...
		$sql = "SELECT * FROM purchase_order WHERE purchase_id = '$purchase_id'";
		...
		$history = "INSERT INTO history(table_report,transaction_type,item_no,detail,history_date,office) 
					VALUES('Purchase Order','Cancellation of P.O. No.','$item_no','P.O. No. $po_no with balance of ".number_format($balance)." pcs of $item_no order by $client has been cancelled','$datetime','$plant')";
