...
?>
	...
	<div id="wrapper" onclick="closeNav();">
		<div id="content">
			<form action="delivery_test.php" method="post" class="form-inline">
...
<?php
	...
	$adjacents = 3;
	$targetpage = "delivery_test.php"; //your file name
	...
	if($lastpage > 1){ 
		...
		$pagination .= "<div class='pagination1'> <ul class='pagination'>";
		$pagination.= "<li class='page-item active'><a class='page-link' href='#'>1</a></li>";
		$pagination.= "</ul></div>\n"; 
	}
 
 	$query = "SELECT *, FORMAT(quantity,0) as quantity, DATE_FORMAT(date_delivery,'%m/%d/%y') as date_delivery
				FROM delivery ".$string." ".$string_date."
				AND 1=1 AND remarks = 'On Delivery' 
...
<?php
	...
	$query = "SELECT *, FORMAT(quantity,0) as quantity, DATE_FORMAT(date_delivery,'%m/%d/%y') as date_delivery 
				FROM delivery ".$string." ".$string_date."
				AND remarks = 'On Delivery' AND 1=1 
				ORDER BY delivery_id DESC
				LIMIT $start, $limit";
    // echo $query;
	$result = mysqli_query($db, $query);
