...
<?php
	if($office == 'delta'){
		echo "<title>Received Orders - Purchase Order Aggregates - Quality Star Concrete Products, Inc.</title>";
	}else{
		echo "<title>Received Orders - Purchase Order Aggregates - Starcrete Manufacturing Corporation</title>";
	}
?>
	<meta charset="utf-8">
...
?>
	...
	<nav class="navbar navbar-default" id="secondary-nav" style="background-color: #0884e4; margin-bottom: 10px;">
		<div class="container-fluid">
			...
			<span style="font-size:25px; color: white;"><a href="purchase_order_aggregates.php" style=" color: white;">Purchase Order Aggregates</a> > Received Orders</span>
	...
	<div id="wrapper" onclick="closeNav();">
		<div id="content">
			<form action="purchase_order_aggregates_success.php" method="post" class="form-inline">
...
?>
					<table class="table table-striped">
						<thead>
							<tr>
								<th colspan="7"><h3>Received Orders</h3></th>
							</tr>
							<tr class="filterable">
...
<?php
		...
		$query = "SELECT p.purchase_order_aggregates_no, p.item_no, CONCAT(FORMAT(p.quantity,0),' ',l.truck) as detail, FORMAT(p.price,0) as price , l.unit, p.supplier_name, DATE_FORMAT(p.date_po_aggregates,'%m/%d/%y') as date_po_aggregates1, p.remarks, p.office, CONCAT(FORMAT(p.received,0), ' ', l.truck) as received 
					...
					WHERE p.item_no = l.item_no AND p.remarks = 'Success' ".$string." ".$string_date."
...
<?php
		...
		$query = "SELECT p.purchase_order_aggregates_no, p.item_no, CONCAT(FORMAT(p.quantity,0),' ',l.truck) as detail, FORMAT(p.price,0) as price, l.unit, p.supplier_name, DATE_FORMAT(p.date_po_aggregates,'%m/%d/%y') as date_po_aggregates1, p.remarks, p.office, CONCAT(FORMAT(p.received,0), ' ', l.truck) as received 
					FROM purchase_order_aggregates p, item_list l
					WHERE p.item_no = l.item_no AND p.remarks = 'Success' ".$string."				
					ORDER BY date_po_aggregates DESC, purchase_order_aggregates_no DESC";
					// echo $query;
		$result = mysqli_query($db, $query);
		if(mysqli_num_rows($result) > 0){
