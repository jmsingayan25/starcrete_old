...
<?php
	if($office == 'delta'){
		echo "<title>Delivered Orders - Purchase Order - Quality Star Concrete Products, Inc.</title>";
	}else{
		echo "<title>Delivered Orders - Purchase Order - Starcrete Manufacturing Corporation</title>";
	}
?>
	<meta charset="utf-8">
...
<script>
 ...
 //    setTimeout(refresh, 10000);
	...
	$(document).ready(function(){
	    ...
	    $.ajax({
		    url: 'purchase_order_pending.php',
		    method: get,
		    data:{
		        var1 : val1
		    },
...
?>
	...
	<nav class="navbar navbar-default" id="secondary-nav" style="background-color: #0884e4; margin-bottom: 10px;">
		<div class="container-fluid">
			...
			<span style="font-size:25px; color: white;"><a href="purchase_order.php" style="color: white;">Purchase Order</a> > Delivered Order</span>
	...
	<div id="wrapper" onclick="closeNav();">
		<div id="content">
			<form action="purchase_order_success.php" method="post" class="form-inline">
...
<?php
	if(isset($_POST['search_table'])){
		...
		// 	$string_date = "AND DATE_FORMAT(date_delivered,'%Y-%m-%d') = '$date'";
		// }
		if($_POST['end_date'] == ''){
			$end_date = date("Y-m-d");
		}else{
		...
		if($_POST['start_date'] == ''){
			...
			// $string_date = "AND DATE_FORMAT(date_delivered,'%Y-%m-%d') = '$date'";
			$string_date = "AND DATE_FORMAT(date_delivered,'%Y-%m-%d') BETWEEN '$date' AND '$end_date'";
		}
?>
					<table class="table table-striped">
						<thead>
							<!-- <tr>
								<th colspan="10"><h3>Delivered Orders</h3></th>
							</tr> -->
							<tr class="filterable">
...
<?php
		if($office == 'head'){
			$string = " AND d.office = '$search_plant'";
		}else{
			$string = " AND d.office = '$office'";
		}
		
		// $query = "SELECT p.purchase_id, p.purchase_order_no, p.client_name, p.item_no, CONCAT(FORMAT(p.quantity,0), ' ', l.unit) as quantity, delivered, backload, balance, address, contact_person, contact_no, DATE_FORMAT(date_delivered,'%c/%d/%y %h:%i %p') as date_delivered, DATE_FORMAT(date_purchase,'%c/%d/%y %h:%i %p') as date_purchase, office, remarks
		//  			FROM purchase_order p, batch_list l
		//  			WHERE p.item_no = l.item_no 
		//  			AND remarks = 'Success' ".$string." ".$string_date."
		// 			ORDER BY date_delivered DESC, purchase_order_no DESC";

		// $query = "SELECT d.po_no_delivery, d.item_no, CONCAT(FORMAT(d.quantity,0), ' ', l.unit) as quantity, d.client_name, d.address, d.contact, d.contact_no, p.date_purchase, d.date_delivery, d.remarks 
		// 			FROM purchase_order p, delivery d, batch_list l
		// 			WHERE d.item_no = l.item_no
		// 			AND d.fk_po_id = p.purchase_id
		// 			AND DATE_FORMAT(p.date_purchase,'%Y-%m-%d') != ''
		// 			AND d.remarks = 'Delivered' ".$string." ".$string_date."
		// 			ORDER BY d.date_delivery DESC, purchase_order_no DESC";
		$query = "SELECT d.*, DATE_FORMAT(p.date_purchase,'%m/%d/%y') as date_purchase, DATE_FORMAT(d.date_delivery,'%m/%d/%y') as date_delivery
					FROM delivery d, purchase_order p
...
<?php
		...
		// 			AND d.fk_po_id = p.purchase_id
		// 			AND d.date_delivery != ''
		// 			AND d.remarks = 'Delivered' ".$string."
		// 			ORDER BY d.date_delivery DESC, purchase_order_no DESC";
