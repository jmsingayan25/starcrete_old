...
<?php
	if($office == 'delta'){
		echo "<title>Received Item List - Transmittal - Quality Star Concrete Products, Inc.</title>";
	}else{
		echo "<title>Received Item List - Transmittal - Starcrete Manufacturing Corporation</title>";
	}
?>
	<meta charset="utf-8">
	...
	<div id="wrapper" onclick="closeNav()">
		<div id="content">
			<div class="row" style="margin: 0px; margin-bottom: 5px;">
				<div class="col-md-6">
					<form action="transmittal.php" method="post" class="form-inline">
					...
					<div class="pull-right">
						<div class="dropdown">
							...
							<ul class="dropdown-menu">
								...
								<li><a href="transmittal_transfer.php">Transferred Items</a></li>
							</ul>
						</div>
					</div>
				</div>
						...
						<thead>
							<tr>
				     		...
				     		<tr class="filters">
								...
								<th class="col-md-1">From Office</th>
				     	...
				     	<tbody>
<?php
	$sql = "SELECT transmittal_no, office, transmittal_unique_id, delivered_by, DATE_FORMAT(transmittal_date,'%m/%d/%y') as transmittal_date1, remarks, from_office, count(transmittal_id) as count 
			FROM transmittal 
			WHERE office = '$office' 
			AND from_office != '$office'".$string_date."
...
?>
							<tr>
								...
								<?php
									if($row['from_office'] == 'Head' || $row['from_office'] == 'head'){
										echo 'Head Office';
									}else{
										echo ucfirst($row['from_office']);
									}
								?>
													</strong>
								</td>
								<td class='col-md-1'>
									<form action="transmittal_received_item.php" method="post">
										<input type="submit" name="received" value="View <?php echo $row['count'] ?> Item(s)" class="btn btn-success btn-sm">
								...
								<td class='col-md-1'><strong><?php echo $row['transmittal_date1']; ?></strong></td>
<?php
...
?>
							<tr>
								...
								<td class='col-md-1'>
									<form action="transmittal_received_item.php" method="post">
										<input type="submit" name="received" value="View <?php echo $row['count'] ?> Item(s)" class="btn btn-success btn-sm btn-sm">
		       							<input type="hidden" name="pass_transmittal_no" value="<?php echo $row['transmittal_no']; ?>">
		       							<input type="hidden" name="pass_from_office" value="<?php echo $row['from_office']; ?>">
		       							<input type="hidden" name="pass_to_office" value="<?php echo $row['office']; ?>">
		       							<input type="hidden" name="pass_id" value="<?php echo $row['transmittal_unique_id']; ?>">
